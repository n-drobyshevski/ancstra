...
/** Event shape expected by the serializer */
export interface GedcomExportEvent {
  id: string;
  eventType: string;
...
/** Full input data for GEDCOM export */
...
/**
 * Serialize tree data to GEDCOM 5.5.1 format.
 *
...
export function serializeToGedcom(data: GedcomExportData, mode: ExportMode): string {
  ...
  const famsForPerson = new Map<string, string[]>(); // personId → familyIds where they are a partner
  const famcForPerson = new Map<string, string[]>(); // personId → familyIds where they are a child
  ...
  // Join with newlines and prepend UTF-8 BOM
  return '\ufeff' + lines.join('\n') + '\n';
}
