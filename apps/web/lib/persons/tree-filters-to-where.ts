...
// WHERE predicates for the tree-table query — mirrors buildPersonsWhere with
// the small differences in URL surface (living: 'living'|'deceased' vs the
// /persons 'alive'|'deceased'; same column names because both read from the
// denormalized `person_summary ps` table).
export function buildTreeTableWhere(
