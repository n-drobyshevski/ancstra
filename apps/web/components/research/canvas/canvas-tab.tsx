...
const ReactFlowProvider = dynamic(
  () => import('@xyflow/react').then(m => m.ReactFlowProvider),
  { ssr: false }
);
...
export function CanvasTab({ personId }: CanvasTabProps) {
  ...
  if (isLoading) {
    return (
      <div className="h-[calc(100dvh-11rem)] md:h-[calc(100vh-16rem)] rounded-lg border border-border bg-muted/30 flex items-center justify-center">
  ...
  return (
    <div className="h-[calc(100dvh-11rem)] md:h-[calc(100vh-16rem)] rounded-lg border border-border overflow-hidden">
      <ReactFlowProvider>
        {drillDownId ? (
          <EvidenceView
