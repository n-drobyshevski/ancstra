...
export function TreeActiveFilters({ topologyReferenceName }: TreeActiveFiltersProps) {
  const t = useTranslations('tree.activeFilters');
  const tSex = useTranslations('persons.facets.sex.values');
  const tLiving = useTranslations('tree.activeFilters.livingValues');
  ...
  if (filters.topologyMode !== 'all' && topologyReferenceName) {
    chips.push({
      key: 'topology',
      label:
        filters.topologyMode === 'ancestors'
          ? t('topologyAncestors', { name: topologyReferenceName })
          : t('topologyDescendants', { name: topologyReferenceName }),
