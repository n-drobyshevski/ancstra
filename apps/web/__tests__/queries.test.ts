...
function createTestDb(): { db: any; sqlite: Database.Database } {
  ...
  sqlite.exec(`
    ...
    CREATE TABLE events (id TEXT PRIMARY KEY, event_type TEXT NOT NULL, date_original TEXT, date_sort INTEGER, date_modifier TEXT DEFAULT 'exact', date_end_sort INTEGER, place_text TEXT, description TEXT, person_id TEXT, family_id TEXT, source_factsheet_id TEXT, contested INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, version INTEGER NOT NULL DEFAULT 1);
...
// Materialize person_summary from underlying tables. Replaces the older
// hand-rolled summary() helper now that getTreeData reads facets directly.
...
describe('getTreeData', () => {
  ...
  it('excludes soft-deleted persons', async () => {
    ...
    n('p1', 'Soft', 'Deleted');
    await materialize();
    // Mark person as deleted AFTER materialization, so person_summary still
    // has the row — exercises getTreeData's anti-join filter.
