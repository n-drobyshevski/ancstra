...
// ---------------------------------------------------------------------------
// Minimal DDL — same 4-source seed pattern as packages/research/src/__tests__/inbox/queries.test.ts.
// ---------------------------------------------------------------------------
...
function makeRequest(params?: string) {
  return new Request(`http://localhost/api/inbox${params ? `?${params}` : ''}`);
}
...
describe('GET /api/inbox', () => {
  it('returns items + counts + hasMore', async () => {
    authSuccess(testDb);
    const res = await GET(makeRequest());
    expect(res.status).toBe(200);
    const body = await res.json() as { items: unknown[]; counts: { total: number; byType: Record<string, number> }; hasMore: boolean };
    ...
    // hasMore: with default limit=50 and our small seed, all items fit on one page
    expect(typeof body.hasMore).toBe('boolean');
    expect(body.hasMore).toBe(false);
  });

  it('parses comma-separated type param', async () => {
    authSuccess(testDb);
    const res = await GET(makeRequest('type=hint,conflict'));
    expect(res.status).toBe(200);
    const body = await res.json() as { items: Array<{ type: string }> };

    expect(body.items.length).toBeGreaterThan(0);
    for (const item of body.items) {
      expect(['hint', 'conflict']).toContain(item.type);
    }
    // no factsheet_draft or ai_proposal
    expect(body.items.some(i => i.type === 'factsheet_draft')).toBe(false);
    expect(body.items.some(i => i.type === 'ai_proposal')).toBe(false);
  });
  ...
  it('caps limit at 200', async () => {
    ...
    // The route caps limit at 200; our seed has far fewer items so this just
    // verifies the server didn't throw and returned a valid array.
    expect(Array.isArray(body.items)).toBe(true);
    expect(body.items.length).toBeLessThanOrEqual(200);
  });

  it('respects threadId=untriaged', async () => {
    authSuccess(testDb);
    const res = await GET(makeRequest('threadId=untriaged'));
    expect(res.status).toBe(200);
    const body = await res.json() as { items: Array<{ threadId: string | null }> };
