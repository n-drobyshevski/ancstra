/**
 * Bundle C integration test: dirty second-promote -> force-repromote round-trip.
 *
 * Exercises promote (first) -> manual edit -> re-promote (409) -> force-repromote
 * using a real in-memory SQLite DB. Verifies that:
 *   - force-repromote returns 200 + mode='force-repromoted' + new personId
 *   - old person row is gone
 *   - new person has the refined fact data (manual edit gone)
 *   - audit trail contains factsheet_force_repromoted event
 */
...
describe('Integration: dirty second-promote -> force-repromote round-trip (Bundle C)', () => {
  ...
  it('promote -> manual edit -> re-promote 409 -> force -> new person, old gone', async () => {
    ...
    const oldPersonId: string = body1.personId;
    expect(oldPersonId).toBeTruthy();

    // Step 3: get promoted_at timestamp
    ...
    // Step 4: simulate manual edit — set events.updated_at 5 seconds after promoted_at
    const promotedAt = new Date(fsRow.promoted_at);
    const futureAt = new Date(promotedAt.getTime() + 5000).toISOString();
    client.prepare(
      `UPDATE events SET place_text = 'Manual', updated_at = ? WHERE person_id = ?`,
    ).run(futureAt, oldPersonId);
    ...
    // Step 6: re-promote -> expect 409 PersonDirty with diffHash
    ...
    const diffHash: string = body2.diffHash;
    expect(diffHash).toMatch(/^[0-9a-f]{64}$/);

    // Step 7: force-repromote with the diffHash from the 409 response
    const res3 = await forcePOST(
      makeForceRequest('F1', { reason: 'discard manual edits', diffHash }),
      { params: Promise.resolve({ id: 'F1' }) },
    );
    expect(res3.status).toBe(200);
    const body3 = await res3.json();
    expect(body3.mode).toBe('force-repromoted');
    const newPersonId: string = body3.personId;
    expect(newPersonId).toBeTruthy();
    expect(newPersonId).not.toBe(oldPersonId); // must be a different person

    // Step 8: verify old person row is gone
    const oldPersonCount = (client.prepare(
      `SELECT COUNT(*) AS n FROM persons WHERE id = ?`,
    ).get(oldPersonId) as { n: number }).n;
    expect(Number(oldPersonCount)).toBe(0);

    // Step 9: verify new person exists and has the refined fact (Moscow), not the manual edit
    const newPersonCount = (client.prepare(
      `SELECT COUNT(*) AS n FROM persons WHERE id = ?`,
    ).get(newPersonId) as { n: number }).n;
    expect(Number(newPersonCount)).toBe(1);

    const newEvent = client.prepare(
      `SELECT place_text FROM events WHERE person_id = ?`,
    ).get(newPersonId) as { place_text: string | null } | undefined;
    // New person should have the refined Moscow fact, not the manual 'Manual' value
    if (newEvent) {
      expect(newEvent.place_text).toBe('Moscow');
    }

    // Step 10: verify audit trail contains factsheet_force_repromoted
    ...
    expect(auditTypes).toContain('factsheet_force_repromoted');
  });
});