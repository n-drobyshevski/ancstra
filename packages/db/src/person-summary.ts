...
export async function refreshSummary(db: FamilyDatabase, personId: string): Promise<void> {
  ...
  const personRows = await db.all<{ id: string; sex: string; is_living: number }>(
  ...
  const nameRows = await db.all<{ given_name: string; surname: string }>(
  ...
  // 3. Query birth event
  const birthRows = await db.all<{ date_original: string | null; date_sort: number | null; place_text: string | null }>(
  ...
  // 4. Query death event
  const deathRows = await db.all<{ date_original: string | null; date_sort: number | null; place_text: string | null }>(
  ...
  // 5. Count spouses
  const spouseRows = await db.all<{ cnt: number }>(
  ...
  // 6. Count children
  const childRows = await db.all<{ cnt: number }>(
  ...
  const parentRows = await db.all<{ partner1_id: string | null; partner2_id: string | null }>(
...
export async function refreshRelatedSummaries(db: FamilyDatabase, personId: string): Promise<void> {
  ...
  const spouseFamilies = await db.all<{ partner1_id: string | null; partner2_id: string | null }>(
  ...
  const parentFamilies = await db.all<{ family_id: string }>(
    sql`SELECT family_id FROM children WHERE person_id = ${personId}`
  );
  for (const link of parentFamilies) {
    const famRows = await db.all<{ partner1_id: string | null; partner2_id: string | null }>(
  ...
  const myFamilies = await db.all<{ id: string }>(
  ...
  for (const fam of myFamilies) {
    const kidRows = await db.all<{ person_id: string }>(
